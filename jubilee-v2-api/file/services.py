...
from .integrations import s3_generate_presigned_post
...
def upload_file(file_obj, file_name, user):
    ...
    presigned_data = service.start(file_name=file_name, file_type=mime_type)
    file = File.objects.get(id=presigned_data.get('id'))
    s3 = boto3.client('s3')
    fields = presigned_data['fields']
    bucket_name = settings.AWS_STORAGE_BUCKET_NAME
    s3.upload_fileobj(BytesIO(file_obj), bucket_name, fields['key'], ExtraArgs={'ACL': fields['acl'], 'ContentType': fields['Content-Type']})
    file.upload_finished_at = timezone.now()
    file.save()
